...
BEGIN
  ...
  SELECT salary INTO curr_salary FROM users WHERE user_id=doctor;
...
BEGIN
  ...
  SELECT CONCAT(firstname, " ", lastname) INTO p_name
    ...
    INSERT INTO cs1434872.logs_admin (content) VALUES (CONCAT(m_name, " for ", p_name, " by ", d_name));
END //
...
BEGIN
  ...
  SELECT CONCAT(firstname, " ", lastname) INTO p_name
    ...
    INSERT INTO cs1434872.admin_logs (content) VALUES (CONCAT(pr_name, " performed on ", p_name, " by ", d_name));
END //

CREATE TRIGGER cs1434872.after_users_insert_trigger AFTER INSERT ON users FOR EACH ROW
BEGIN
	INSERT INTO  cs1434872.new_user_logs  (content) VALUES (CONCAT("User '", NEW.username, "' created"));
END //
...
BEGIN
    ...
    THEN
		INSERT INTO  cs1434872.raise_logs  (content) VALUES (CONCAT("Salary of doctor with id ", NEW.user_id, " changed from ", OLD.salary, " to ", NEW.salary));
    END IF;
END //
...
BEGIN
	IF NEW.instock < 6 THEN
		INSERT INTO  cs1434872.notifications  (content) VALUES (CONCAT("Medication ", NEW.name, " stock is ", NEW.instock));
	END IF;
END //
